...
class candidate:
    def __init__(self, DM, sigma, time, bin, downfact,beta):
        ...
        self.beta = beta
    def __str__(self):
        return "%7.2f %7.2f %13.6f %10d     %3d %0.2f\n"%\
               (self.DM, self.sigma, self.time, self.bin, self.downfact, self.beta)
...
def read_singlepulse_files(infiles, threshold, T_start, T_end):
    ...
    for ii, infile in enumerate(infiles):
        if infile.endswith(".scatteredsearch"):
            filenmbase = infile[:infile.rfind(".scatteredsearch")]
        else:
            filenmbase = infile
...
def main():
    ...
    if args[0].endswith(".scatteredsearch"):
        filenmbase = args[0][:args[0].rfind(".scatteredsearch")]
        dosearch = False
    ...
    if (opts.makeplot):
        ...
        #            scipy.stats.rv_histogram(Num.histogram(snrs,bins=
        #                                  int(maxsnr-opts.threshold+1),range=(
        ...
        if pgplot_device:
            ...
            if (opts.T_start > 0.0 or opts.T_end < obstime):
                ppgplot.pgopen(short_filenmbase+'_%.0f-%.0fs_scatteredsearch.ps/VPS'%
                               (opts.T_start, opts.T_end))
            else:
                ppgplot.pgopen(short_filenmbase+'_scatteredsearch.ps/VPS')
