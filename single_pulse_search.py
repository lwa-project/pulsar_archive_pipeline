...
def main():
    ...
    if (opts.makeplot):
        ...
        snrs = [x for x in snrs if ~Num.isinf(x)]
        if snrs:
            print(max(snrs))
        ...
        ppgplot.pgswin(min(DMs)-0.0001, max(DMs)+0.0001, 0.0, 1.1*max(num_v_DM+[1]))
        ppgplot.pgsch(0.8)
        ...
        ppgplot.pgswin(min(DMs)-0.0001, max(DMs)+0.0001, opts.threshold, maxsnr)
        ppgplot.pgsch(0.8)
        ppgplot.pgbox("BCNST", 0, 0, "BCNST", 0, 0)
        ppgplot.pgmtxt('B', 2.5, 0.5, 0.5, r"DM (pc cm\u-3\d)")
        ...
        ppgplot.pgswin(opts.T_start, opts.T_end, min(DMs)-0.0001, max(DMs)+0.0001)
        ppgplot.pgsch(0.8)
        ppgplot.pgbox("BCNST", 0, 0, "BCNST", 0, 0)
